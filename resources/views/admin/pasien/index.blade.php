...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#frm-edit').on('submit', function(e) {
			...
			$.post("{{ route('ambilUpdatePasien') }}", data, function() {
		...
		$('#datatable').on('click', '.btn-hapus', function(e) {
	              var id = $(this).data('id');
	                 $.confirm({
	                    ...
	                    buttons: {
	                        confirm: function () {
	                            $.get("{{ route('ambilHapusPasien') }}", {id: id}, function (data) {
		...
		$('#frm-tambah').on('submit', function(e) {
			...
			var data = $(this).serialize();
			 $.ajax({
	          url: '{{ route('ambilPasienTerdaftar') }}',
	          data: data,
	          method:'POST',
	          dataType: 'json',
	          async:false
				...
				window.open("/admin/no-antrian-pasien/pasien_id="+data.id, "_newtab")
